...
%energy
j = 1;count = 0;
energy = zeros(int16(length(y)/256+1), 1);    %初始化0矩陣
for i = 1:frameNumber            
        energy(j) = energy(j) + frame(i);     %設定一個frame為256個向量
...
zerotime = (1:length(zerorate))/fs*256;    %對應frame的時間

%pitch 音高，使用acf法
j = 1;count = 1;
acfs = zeros(int16(length(y)/256+1), 1);
...
for i = 1:int16(length(y)/256)-1    %使用acf:將一段音框平移一段時間，然後跟原本的音框作自相關運算
    for j=0:256-1
        sum = 0;
        for k=1:256-j
        ...
        pitch(i) = max(acf(20:length(acf)));    %開頭的最大值周圍忽略
end
acftime = (1:length(pitch))/fs*256;   %對應frame的時間
...
%以紅線標出音檔的end-point
line(time(speech(1)*256)*[1 1], [-1, 1], 'color', 'r');
line(time(speech(end)*256)*[1 1], [-1, 1], 'color', 'r');
